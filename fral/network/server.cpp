...
namespace fral {
    ...
    Status server::sync(ServerContext *context, ServerReader <Allocation> *stream, Empty *complete) {
        Allocation alloc;
    ...
    Status server::shutdown(ServerContext *context, const Empty *finish, Empty *ok) {
        keepAlive = false;
        cv.notify_one();
        return Status::OK;
    }
